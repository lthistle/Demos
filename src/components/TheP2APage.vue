...
<div>
  ...
  <div class="row" v-for="customers in chunkedCustomers">
      <div id='yup' class="column" v-for="customer in customers">
        ...
        <span id='personname'><b>Position</b>:  {{customer.job}}<br></span>
...
export default {
  ...
  data() {
    return {
      ...
      pname: 'stephen huan',
      plist: [new Politician('Washington','DC','Tim Kaine','timkaine','Democrat',"http://upload.wikimedia.org/wikipedia/commons/a/a3/Tim_Kaine_116th_official_portrait.jpg", "Senator"), new Politician('Washington','DC','Don Beyer', 'https://beyer.house.gov/rss.xml', 'Democrat', "https://upload.wikimedia.org/wikipedia/commons/e/e3/Don_Beyer%2C_official_114th_Congress_photo_portrait.jpeg", "Representative"), new Politician('Richmond','VA','Justin Fairfax','https://www.facebook.com/JustinEFairfax/', 'Democrat', "https://upload.wikimedia.org/wikipedia/commons/8/8d/Justin_Fairfax_crop.jpg","Lt. Governor"), new Politician('Richmond','VA','Dick Saslaw', 'SenSaslaw', 'Democrat', "https://upload.wikimedia.org/wikipedia/commons/0/0f/Sen._Saslaw_2018.jpg", "Senator"), new Politician('Richmond','VA','Kaye Kory','kayekory','Democrat',"https://upload.wikimedia.org/wikipedia/commons/4/4f/Kaye_Kory_2010.jpg", "Delegate"), new Politician('Richmond','VA','Ralph Northam','GovernorVA','Democrat',"https://upload.wikimedia.org/wikipedia/commons/7/72/Governor_Ralph_Northam_Gives_Inaugural_Address_%2839348612584%29_%28cropped%29.jpg", "Governor"), new Politician('Washington','DC','Donald Trump','POTUS','Republican',"https://upload.wikimedia.org/wikipedia/commons/5/56/Donald_Trump_official_portrait.jpg", "President"), new Politician('Washington','DC','Mike Pence','VP','Republican',"https://upload.wikimedia.org/wikipedia/commons/b/b9/Mike_Pence_official_Vice_Presidential_portrait.jpg", "Vice President"), new Politician('Washington','DC','Mark Warner','W000805','Democrat',"https://upload.wikimedia.org/wikipedia/commons/0/0c/Mark_Warner_113th_Congress_photo.jpg", "Senator")]
    }
  },
  methods: {
    getPolitician() {
      ...
      Axios.get(endpoint, {
        ...
        for(let i=0; i< data.length; i++) {
          ...
          let job = data[i].office_details.position
          var params_wiki = {
            'format': 'json',
            'action': 'query',
            'prop': 'pageimages',
          ...
          Axios.get('https://en.wikipedia.org/w/api.php', {
            ...
            var pageKeys = Object.keys(pages)
              if (pages[pageKeys[0]].original == null) {
                image = "https://i.ibb.co/Qd6b6q8/nopfp.png"
              }
              else {
                image = pages[pageKeys[0]].original.source
              }
              vm.plist.push(new Politician(city, state, name, social, party, image, job))
            })
